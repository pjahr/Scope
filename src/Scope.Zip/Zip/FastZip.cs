...
using static Scope.Zip.Zip.Compression.Deflater;
...
namespace Scope.Zip.Zip
{
  ...
  public class FastZipEvents
  {
    ...
    public TimeSpan ProgressInterval { get => progressInterval_; set => progressInterval_ = value; }
  ...
  public class FastZip
  {
    ...
    public void CreateZip(Stream outputStream,
                          string sourceDirectory,
                          bool recurse,
                          string fileFilter,
                          string directoryFilter)
    ...
    public void ExtractZip(Stream inputStream,
                           string targetDirectory,
                           Overwrite overwrite,
                           ConfirmOverwriteDelegate confirmDelegate,
                           string fileFilter,
      ...
      if (e.ContinueRunning)
      {
        try
        {
          ...
          using (FileStream stream =
            File.Open(e.Name, FileMode.Open, FileAccess.Read, FileShare.Read))
          {
        ...
        if (continueRunning_)
        {
          try
          {
            using (FileStream outputStream = File.Create(targetName))
            {
              ...
              if (events_ != null && events_.Progress != null)
              {
                StreamUtils.Copy(zipFile_.GetInputStream(entry),
                                 outputStream,
                                 buffer_,
                                 events_.Progress,
                                 events_.ProgressInterval,
            ...
            if (RestoreAttributesOnExtract
                && entry.IsDOSEntry
                && entry.ExternalFileAttributes != -1)
            {
