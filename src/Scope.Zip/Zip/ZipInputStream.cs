  ...
  public class ZipInputStream : InflaterInputStream
  {
    ...
    public ZipEntry GetNextEntry()
    {
      ...
      entry = new ZipEntry(name, versionRequiredToExtract)
              {
                Flags = flags, CompressionMethod = (CompressionMethod) method
              };
    ...
    private int InitialRead(byte[] destination, int offset, int count)
    {
      ...
      if (entry.IsCrypted)
      {
        ...
        {
          throw new ZipException(string.Format("Entry compressed size {0} too small for encryption",
                                               csize));
        }
      }
      else
      ...
      switch (method)
      {
        case (int) CompressionMethod.Deflated:
          ...
          if (count <= 0)
          {
            ...
            if ((flags & 8) == 0
                && (inf.TotalIn != csize && csize != 0xFFFFFFFF && csize != -1
                    || inf.TotalOut != size))
            {
