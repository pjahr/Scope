  ...
  public class TarBuffer
  {
    ...
    */
    ...
    public void WriteBlock(byte[] block)
    {
      ...
      {
        string errorText =
          string.Format("TarBuffer.WriteBlock - block to write has length '{0}' which is not the block size of '{1}'",
                        block.Length,
                        BlockSize);
    ...
    public void WriteBlock(byte[] buffer, int offset)
    {
      ...
      {
        string errorText =
          string.Format("TarBuffer.WriteBlock - record has length '{0}' with offset '{1}' which is less than the record size of '{2}'",
                        buffer.Length,
                        offset,
                        recordSize);
        throw new TarException(errorText);
