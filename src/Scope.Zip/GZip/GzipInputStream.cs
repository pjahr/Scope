...
namespace Scope.Zip.GZip
{
  ...
  public class GZipInputStream : InflaterInputStream
  {
    ...
    /// </param>
    public GZipInputStream(Stream baseInputStream, int size) : base(baseInputStream,
                                                                    new Inflater(true),
                                                                    size) { }
    ...
    private void ReadFooter()
    {
      ...
      // Calculate CRC
      int crcval = (footer[0] & 0xff)
                   | ((footer[1] & 0xff) << 8)
                   | ((footer[2] & 0xff) << 16)
                   | (footer[3] << 24);
      if (crcval != (int) crc.Value)
      {
        throw new GZipException("GZIP crc sum mismatch, theirs \""
                                + crcval
                                + "\" and ours \""
                                + (int) crc.Value);
      }
